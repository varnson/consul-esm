	...
	for {
		...
		case newNodes := <-nodeCh:
			if len(newNodes) != len(nodes) {
				...
				a.logger.Info("Now running probes for external nodes", "count", hclog.Fmt("%d ", len(newNodes)))
			}
			nodes = newNodes
			shuffleNodes(nodes)
			index = 0
		...
		if len(nodes) == 0 {
			a.logger.Debug("No nodes to probe", "will retry in ", hclog.Fmt("%s", retryTime.String()))
...
func (a *Agent) runNodePing(node *api.Node) {
	...
	if err == nil {
		if err := a.updateHealthyNode(node, kvClient, key, kvPair); err != nil {
			a.logger.Warn("error updating node: ", "error", hclog.Fmt("%v", err))
		}
...
func (a *Agent) nodeTicker(numNodes int) *time.Ticker {
	...
	a.logger.Debug("Now waiting between node pings", "time", hclog.Fmt(" %s", waitTime.String()))
...
func (a *Agent) updateHealthyNode(node *api.Node, kvClient *api.KV, key string, kvPair *api.KVPair) error {
	...
	if !toUpdate {
		a.logger.Trace("Debounce: skipping healthy node status update for node", "node", hclog.Fmt(" %s", node.Node))
		return nil
	}

	a.logger.Trace("Debounce: updating healthy node status. ", "node", hclog.Fmt("%s", node.Node), "status", hclog.Fmt("%s", status))
...
func (a *Agent) updateHealthyNodeTxn(node *api.Node, kvClient *api.KV, key string, kvPair *api.KVPair) error {
	...
	if kvPair != nil {
		...
		a.logger.Trace("Deleting KV entry",  "key", hclog.Fmt("%s", key))
	}
...
func (a *Agent) updateFailedNode(node *api.Node, kvClient *api.KV, key string, kvPair *api.KVPair) error {
	...
	toUpdate := a.shouldUpdateNodeStatus(node.Node, status)
	if !toUpdate {
		a.logger.Trace("Debounce: skipping failed node status update for node", "node", hclog.Fmt(" %s", node.Node))
		return nil
	}

	a.logger.Trace("Debounce: updating failed node status.", "node", hclog.Fmt(" %s", node.Node), "status", hclog.Fmt(" %s", status))
...
func (a *Agent) updateFailedNodeTxn(node *api.Node, kvClient *api.KV, key string, kvPair *api.KVPair) error {
	...
	if kvPair == nil {
		...
		a.logger.Trace("Writing KV entry for key.", "key", hclog.Fmt("%s", key))
	} else {
		...
		if time.Since(criticalStart) > a.config.NodeReconnectTimeout {
			...
			if existing != nil && existing.Node != nil {
				...
				a.logger.Debug("Deregistering node ", "node", hclog.Fmt("%q", node.Node))
			}
...
func (a *Agent) updateNodeCheck(node *api.Node, ops api.TxnOps, status, output string) error {
	...
	a.logger.Trace("Updating external health check for node ", "node", hclog.Fmt("%q", node.Node))
...
func (a *Agent) updateNodeCoordinate(node *api.Node, rtt time.Duration) error {
	if a.config.DisableCoordinateUpdates {
		a.logger.Trace("Debounce: skipping coordinate update for node ", "node", hclog.Fmt("%s", node.Node))
		return nil
	}
	...
	if len(coords) > 0 && coord.Coord.DistanceTo(newCoord) <= time.Millisecond {
		a.logger.Trace("Skipped update for coordinates, ", "info", hclog.Fmt("node %q change %q not significant", node.Node, coord.Coord.DistanceTo(newCoord)))
		return nil
	}
	...
	a.logger.Info("Updated coordinates for ", "node", hclog.Fmt("%q with distance %q from previous", node.Node, coord.Coord.DistanceTo(newCoord)))
	return nil
}
