...
LEADER_WAIT:
	...
	if lock == nil {
		...
		if err != nil {
			a.logger.Error("Error trying to create leader lock (will retry): ", "error", hclog.Fmt("%v", err))
	...
	if err != nil {
		if err == api.ErrLockHeld {
			a.logger.Error("Unable to use leader lock that was held previously and presumed lost, giving up the lock (will retry): ", "error", hclog.Fmt("%v", err))
			lock.Unlock()
			time.Sleep(retryTime)
			goto LEADER_WAIT
		} else {
			a.logger.Error("Error trying to get leader lock (will retry): ", "error", hclog.Fmt("%v", err))
			time.Sleep(retryTime)
			goto LEADER_WAIT
		}
	}
...
WATCH_NODES_WAIT:
	for {
		...
		if !reflect.DeepEqual(healthNodes, prevHealthNodes) || !reflect.DeepEqual(pingNodes, prevPingNodes) {
			a.logger.Info("", "Rebalanced ", hclog.Fmt("%d external nodes across %d ESM instances", len(externalNodes), len(healthyInstances)))
...
func (a *Agent) watchExternalNodes(nodeCh chan []*api.Node, stopCh <-chan struct{}) {
	...
	for {
		...
		if err != nil {
			a.logger.Warn("Error getting external node list: ", "error", hclog.Fmt("%v", err))
			continue
		}
		...
		a.logger.Info("Updating external node list, ", "info", hclog.Fmt("set to %d items", len(externalNodes)))
...
func (a *Agent) watchServiceInstances(instanceCh chan []*api.ServiceEntry, stopCh <-chan struct{}) {
	...
	for {
		...
		if err != nil {
			a.logger.Warn("[WARN] Error querying for health check info: ", "error", hclog.Fmt("%v", err))
			continue
		}
